...
from exp_baselines.bayesmark.data import METRICS_LOOKUP, ProblemType, load_data
import pickle
...
class SklearnModelCustom(TestFunction):
    ...
    _CUSTOM_DATASET = [
        "maggic", "seer", "cutract", 'Rosenbrock', 'Griewank', 'KTablet', 'cifar10'
    ]
    ...
    def __init__(self, model, dataset, metric, shuffle_seed=0, data_root=None):
        ...
        self.model_name = SklearnModelCustom._LIST_MODELS_CLF[model] if self.is_classifier else \
        SklearnModelCustom._LIST_MODELS_REG[model]
        self.model_name2 = SklearnModelCustom._LIST_MODELS_CLF2[model] if self.is_classifier else \
        SklearnModelCustom._LIST_MODELS_REG2[model]
