...
import syne_tune_local
from syne_tune_local import Tuner
from syne_tune_local.remote.estimators import (
    instance_sagemaker_estimator,
)
from syne_tune_local.remote.constants import DEFAULT_CPU_INSTANCE
from syne_tune_local.remote.remote_metrics_callback import RemoteTuningMetricsCallback
from syne_tune_local.backend.sagemaker_backend.sagemaker_utils import (
    add_syne_tune_dependency,
    get_execution_role,
)
from syne_tune_local.constants import ST_REMOTE_UPLOAD_DIR_NAME
from syne_tune_local.util import s3_experiment_path
...
class RemoteLauncher:
    ...
    :param tuner: Tuner that should be run remotely on a ``instance_type``
        instance. Note that :class:`~syne_tune_local.StoppingCriterion` should be used
        for the :class:`~syne_tune_local.Tuner` rather than a lambda function to ensure
        serialization.
    ...
    :param s3_path: S3 base path used for checkpointing, outputs of tuning
        ...
        Defaults to :func:`~syne_tune_local.util.s3_experiment_path`
    :param no_tuner_logging: If ``True``, the logging level for ``syne_tune_local.tuner``
        is set to ``logging.ERROR``. Defaults to ``False``
    :param publish_tuning_metrics: If ``True``, a number of tuning metrics (see
        :class:`~syne_tune_local.remote.remote_metrics_callback.RemoteTuningMetricsCallback`)
    ...
    def upload_dir(self) -> Path:
        return Path(syne_tune_local.__path__[0]).parent / ST_REMOTE_UPLOAD_DIR_NAME
...
def syne_tune_image_uri() -> str:
    ...
    try:
        ...
        script_path = Path(syne_tune_local.__path__[0]).parent / "container"
        logger.warning(
        ...
        subprocess.run(
            ...
            cwd=Path(syne_tune_local.__path__[0]).parent / "container",
        )
