...
from syne_tune_local import Reporter
from syne_tune_local.backend.trial_status import Trial
from syne_tune_local.config_space import Domain
from syne_tune_local.constants import MAX_METRICS_SUPPORTED_BY_SAGEMAKER
...
class RemoteTuningMetricsCallback(TunerCallback):
    """
    Reports metrics related to the experiment run by :class:`~syne_tune_local.Tuner`.
    ...
    * If ``config_space`` is given, then for each hyperparameter ``name`` in
      ...
      However, at most :const:`~syne_tune_local.constants.MAX_METRICS_SUPPORTED_BY_SAGEMAKER`
      are supported
    """
    ...
    def register_metrics_with_estimator(self, estimator: EstimatorBase):
        ...
        :const:`~syne_tune_local.constants.MAX_METRICS_SUPPORTED_BY_SAGEMAKER`. Otherwise,
