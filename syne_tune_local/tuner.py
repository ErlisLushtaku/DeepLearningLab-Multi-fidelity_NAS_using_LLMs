...
from syne_tune_local.backend.trial_status import Status, Trial, TrialResult
...
from syne_tune_local.optimizer.scheduler import SchedulerDecision, TrialScheduler
...
from syne_tune_local.results_callback import StoreResultsCallback
from syne_tune_local.tuning_status import TuningStatus, print_best_metric_found
from syne_tune_local.util import (
    RegularCallback,
...
class Tuner:
    ...
    :param sleep_time: Time to sleep when all workers are busy. Defaults to
        :const:`~syne_tune_local.constants.DEFAULT_SLEEP_TIME`
    ...
    :param trial_backend_path: If this is given, the path of ``trial_backend``
        ...
        If the backend is :class:`~syne_tune_local.backend.LocalBackend` and the
