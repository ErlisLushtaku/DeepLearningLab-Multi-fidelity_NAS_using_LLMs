...
from syne_tune_local.experiments.benchmark_definitions.common import BenchmarkDefinition
...
def set_logging_level(args: ConfigDict):
    if args.verbose:
        ...
        logging.getLogger("syne_tune_local.optimizer.schedulers").setLevel(logging.WARNING)
        logging.getLogger("syne_tune_local.backend").setLevel(logging.WARNING)
        logging.getLogger(
            "syne_tune_local.backend.simulator_backend.simulator_backend"
...
def get_metadata(
    ...
    """Returns default value for ``metadata`` passed to :class:`~syne_tune_local.Tuner`.
