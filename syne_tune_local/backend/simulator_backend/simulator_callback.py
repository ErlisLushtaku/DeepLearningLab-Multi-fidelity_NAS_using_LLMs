...
from syne_tune_local.results_callback import StoreResultsCallback, ExtraResultsComposer
from syne_tune_local.backend.simulator_backend.simulator_backend import SimulatorBackend
from syne_tune_local import Tuner
from syne_tune_local.constants import ST_TUNER_TIME
from syne_tune_local import StoppingCriterion
...
class SimulatorCallback(StoreResultsCallback):
    """
    Callback to be used in :meth:`~syne_tune_local.Tuner.run` in order to support the
    :class:`~syne_tune_local.backend.simulator_backend.SimulatorBackend`.
    ...
    defined in the backend). The real sleep time in :class:`~syne_tune_local.Tuner`
    must be 0.
    ...
    :class:`~syne_tune_local.backend.SimulatorBackend` adding ``ST_TUNER_TIME``
    entries to each result it receives.

    Third (and most subtle), we need to make sure the stop criterion in
    :meth:`~syne_tune_local.Tuner.run` is using simulated time instead of real time when making
    a decision based on ``max_wallclock_time``. By default,
    :class:`~syne_tune_local.StoppingCriterion` takes ``TuningStatus`` as an input,
    ...
    both :class:`~syne_tune_local.Tuner` and ``TuningStatus`` independent of the time
    keeper.
