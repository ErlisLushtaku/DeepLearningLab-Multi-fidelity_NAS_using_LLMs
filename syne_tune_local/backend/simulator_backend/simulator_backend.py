...
from syne_tune_local.report import retrieve
from syne_tune_local.backend.trial_backend import (
    TrialAndStatusInformation,
    TrialIdAndResultList,
)
from syne_tune_local.backend.local_backend import LocalBackend
from syne_tune_local.util import dump_json_with_numpy
from syne_tune_local.backend.trial_status import TrialResult, Status, Trial
from syne_tune_local.backend.simulator_backend.time_keeper import SimulatedTimeKeeper
from syne_tune_local.backend.simulator_backend.events import (
    SimulatorState,
    StartEvent,
    CompleteEvent,
    StopEvent,
...
class SimulatorBackend(LocalBackend):
    ...
    Time is advanced both by :meth:`~syne_tune_local.Tuner.run` waiting, and by non-negligible
    ...
    .. note::
       ...
       ::class:`~syne_tune_local.blackbox_repository.BlackboxRepositoryBackend` after
    ...
    :param tuner_sleep_time: Effective sleep time in
        :meth:`~syne_tune_local.Tuner.run`. This information is needed in
        :class:`~syne_tune_local.backend.simulator_backend.SimulatorCallback`.
        Defaults to :const:`~syne_tune_local.tuner.DEFAULT_SLEEP_TIME`
    """
