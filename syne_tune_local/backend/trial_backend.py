...
from syne_tune_local.backend.trial_status import TrialResult, Trial, Status
from syne_tune_local.constants import ST_WORKER_TIMESTAMP
...
class TrialBackend:
    ...
    :param pass_args_as_json: Normally, the hyperparameter configuration is
        ...
        :const:`~syne_tune_local.constants.ST_CONFIG_JSON_FNAME_ARG`. The trial
    ...
    def busy_trial_ids(self) -> List[Tuple[int, str]]:
        ...
        :const:`~syne_tune_local.backend.trial_status.Status.in_progress` or
        :const:`~syne_tune_local.backend.trial_status.Status.stopping`.
    ...
    def set_path(
        ...
        :param results_root: The local folder that should contain the results of
            the tuning experiment. Used by :class:`~syne_tune_local.Tuner` to indicate
            a desired path where the results should be written to. This is used
            to unify the location of backend files and :class:`~syne_tune_local.Tuner`
    ...
    def on_tuner_save(self):
        """
        Called at the end of :meth:`~syne_tune_local.Tuner.save`.
        """
        pass
