...
from syne_tune_local.backend.trial_status import TrialResult
...
from syne_tune_local.util import experiment_path, random_string, s3_experiment_path
...
def add_metric_definitions_to_sagemaker_estimator(
    ...
    name is compatible with how :class:`~syne_tune_local.Reporter` outputs metric
    values.
...
def add_syne_tune_dependency(sm_estimator):
    ...
    sm_estimator.dependencies = sm_estimator.dependencies + [
        str(Path(syne_tune_local.__path__[0]))
    ]
...
def backend_path_not_synced_to_s3() -> Path:
    ...
    constructing the :class`~syne_tune_local.Tuner`.
