...
from syne_tune_local.backend.trial_backend import TrialBackend, BUSY_STATUS
...
from syne_tune_local.util import s3_experiment_path, dump_json_with_numpy
from syne_tune_local.backend.trial_status import TrialResult, Status
...
class SageMakerBackend(TrialBackend):
    ...
    :const:`~syne_tune_local.constants.ST_INSTANCE_TYPE` and
    :const:`~syne_tune_local.constants.ST_INSTANCE_COUNT`. If these are given in the
    ...
    :class:`~syne_tune_local.backend.trial_backend.TrialBackend`:
    ...
    def on_tuner_save(self):
        # Re-initialize the session after :class:`~syne_tune_local.Tuner` is serialized
        self.initialize_sagemaker_session()
