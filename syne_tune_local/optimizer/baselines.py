...
from syne_tune_local.optimizer.schedulers import (
    FIFOScheduler,
    HyperbandScheduler,
    PopulationBasedTraining,
)
from syne_tune_local.optimizer.schedulers.multiobjective import (
    MOASHA,
    MultiObjectiveRegularizedEvolution,
    NSGA2Searcher,
    LinearScalarizedScheduler,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.estimator import Estimator
from syne_tune_local.optimizer.schedulers.searchers.regularized_evolution import (
    RegularizedEvolution,
)
from syne_tune_local.optimizer.schedulers.synchronous import (
    SynchronousGeometricHyperbandScheduler,
    GeometricDifferentialEvolutionHyperbandScheduler,
)
from syne_tune_local.optimizer.schedulers.transfer_learning import (
    TransferLearningTaskEvaluations,
)
from syne_tune_local.optimizer.schedulers.random_seeds import RandomSeedGenerator
from syne_tune_local.try_import import (
...
from syne_tune_local.util import dict_get
...
def _random_seed_from_generator(random_seed: int) -> int:
    ...
    :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler` is seeded in the same
...
class RandomSearch(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.RandomSearcher`
    ...
    :param kwargs: Additional arguments to
        :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`
    """
...
class GridSearch(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.GridSearcher`
    for ``kwargs["search_options"]`` parameters.
...
class BayesianOptimization(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.GPFIFOSearcher`
    for ``kwargs["search_options"]`` parameters.
...
class ASHA(HyperbandScheduler):
    ...
    See also :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler` for
    ``kwargs`` parameters.

    :param config_space: Configuration space for evaluation function
    :param metric: Name of metric to optimize
    :param resource_attr: Name of resource attribute
    :param kwargs: Additional arguments to :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`
    """
...
class MOBSTER(HyperbandScheduler):
    ...
    :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`.
    ...
    * :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler` for ``kwargs``
      parameters
...
class HyperTune(HyperbandScheduler):
    ...
    * :class:`~syne_tune_local.optimizer.schedulers.searchers.hypertune.HyperTuneSearcher`
      for ``kwargs["search_options"]`` parameters
    * :class:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.hypertune.gp_model.HyperTuneIndependentGPModel`
      for implementation
...
class DyHPO(HyperbandScheduler):
    ...
    * We implement a hybrid of DyHPO with the asynchronous successive halving
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler` for ``kwargs``
    parameters, and
    :class:`~syne_tune_local.optimizer.schedulers.searchers.GPMultiFidelitySearcher`
    ...
    * ``probability_sh``: See comment. The smaller this probability, the closer
      ...
      :const:`~syne_tune_local.optimizer.schedulers.searchers.dyhpo.hyperband_dyhpo.DEFAULT_SH_PROBABILITY`.
...
class PASHA(HyperbandScheduler):
    ...
    more useful, see also :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`.
...
class BOHB(HyperbandScheduler):
    ...
    See
    :class:`~syne_tune_local.optimizer.schedulers.searchers.kde.MultiFidelityKernelDensityEstimator`
...
class SyncHyperband(SynchronousGeometricHyperbandScheduler):
    ...
    The latter is more useful, see also :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`.
    ...
    :param resource_attr: Name of resource attribute
    :param kwargs: Additional arguments to
        :class:`~syne_tune_local.optimizer.schedulers.synchronous.SynchronousGeometricHyperbandScheduler`
    """
...
class BORE(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.bore.Bore`
    for ``kwargs["search_options"]`` parameters.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.bore import Bore
        except ImportError:
...
class ASHABORE(HyperbandScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.bore.MultiFidelityBore`
    for ``kwargs["search_options"]`` parameters.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.bore import MultiFidelityBore
        except ImportError:
            logging.info(try_import_bore_message())
            raise
...
class BoTorch(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.botorch.BoTorchSearcher`
    for ``kwargs["search_options"]`` parameters.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.botorch import BoTorchSearcher
        except ImportError:
...
class REA(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.regularized_evolution.RegularizedEvolution`
    for ``kwargs["search_options"]`` parameters.

    :param config_space: Configuration space for evaluation function
    :param metric: Name of metric to optimize
    :param population_size: See
        :class:`~syne_tune_local.optimizer.schedulers.searchers.RegularizedEvolution`.
        Defaults to 100
...
class MORandomScalarizationBayesOpt(FIFOScheduler):
    """
    Uses :class:`~syne_tune_local.optimizer.schedulers.multiobjective.MultiObjectiveMultiSurrogateSearcher`
    with one standard GP surrogate model per metric (same as in
    :class:`BayesianOptimization`, together with the
    :class:`~syne_tune_local.optimizer.schedulers.multiobjective.MultiObjectiveLCBRandomLinearScalarization`
    ...
    :param estimators: Use these surrogate models instead of the default GP
        one. Optional
    :param kwargs: Additional arguments to
        :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`. Here,
...
class MOLinearScalarizationBayesOpt(LinearScalarizedScheduler):
    """
    Uses :class:`~syne_tune_local.optimizer.schedulers.multiobjective.LinearScalarizedScheduler`
...
class ConstrainedBayesianOptimization(FIFOScheduler):
    ...
    See :class:`~syne_tune_local.optimizer.schedulers.searchers.constrained.ConstrainedGPFIFOSearcher`
    for ``kwargs["search_options"]`` parameters.
...
class ZeroShotTransfer(FIFOScheduler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            from syne_tune_local.optimizer.schedulers.transfer_learning import zero_shot
        except ImportError:
...
class ASHACTS(HyperbandScheduler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            from syne_tune_local.optimizer.schedulers.transfer_learning.quantile_based.quantile_based_searcher import (
...
class KDE(FIFOScheduler):
    ...
    Combines :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler` with TPE-like
    Bayesian optimization, using kernel density estimators.

    See
    :class:`~syne_tune_local.optimizer.schedulers.searchers.kde.KernelDensityEstimator`
    for ``kwargs["search_options"]`` parameters.

    :param config_space: Configuration space for evaluation function
    :param metric: Name of metric to optimize
    :param kwargs: Additional arguments to :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`
    """
...
class CQR(FIFOScheduler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.conformal.surrogate_searcher import (
                SurrogateSearcher,
            )
        except ImportError:
            logging.info(try_import_blackbox_repository_message())
...
try:
    from syne_tune_local.optimizer.schedulers.smac_scheduler import SMACScheduler
