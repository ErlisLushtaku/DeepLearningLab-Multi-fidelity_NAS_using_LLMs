...
from syne_tune_local.config_space import (
    Domain,
    is_log_space,
    Categorical,
    Ordinal,
...
def _impute_default_config(
    ...
    or log). For :class:`~syne_tune_local.config_space.Categorical`, we pick the first
    entry of ``categories``.

    :param default_config: Configuration to be imputed
    :param config_space: Configuration space
    :return: Imputed configuration. If ``default_config`` has entries with
        values not being :class:`~syne_tune_local.config_space.Domain`, they are not
        included
    """
    new_config = dict()
...
def impute_points_to_evaluate(
    ...
    :class:`~syne_tune_local.optimizer.schedulers.searchers.BaseSearcher`. Each
...
class BaseSearcher:
    ...
    .. note::
       ...
       start from
       :class:`~syne_tune_local.optimizer.scheduler.searcher.StochasticAndFilterDuplicatesSearcher`
       or :class:`~syne_tune_local.optimizer.scheduler.searcher.StochasticSearcher`,
    ...
    def configure_scheduler(self, scheduler):
        ...
        :type scheduler: :class:`~syne_tune_local.optimizer.schedulers.TrialScheduler`
        """
    ...
    def on_trial_result(
        ...
        :param trial_id: See :meth:`~syne_tune_local.optimizer.schedulers.TrialScheduler.on_trial_result`
        :param config: See :meth:`~syne_tune_local.optimizer.schedulers.TrialScheduler.on_trial_result`
        :param result: See :meth:`~syne_tune_local.optimizer.schedulers.TrialScheduler.on_trial_result`
    ...
    def debug_log(self) -> Optional[DebugLogPrinter]:
        ...
        :class:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.utils.debug_log.DebugLogPrinter`.
