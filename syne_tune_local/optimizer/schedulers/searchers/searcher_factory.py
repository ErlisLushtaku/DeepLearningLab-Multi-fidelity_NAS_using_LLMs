...
def searcher_factory(searcher_name, **kwargs) -> BaseSearcher:
    ...
    :param kwargs: Argument to
        :class:`~syne_tune_local.optimizer.schedulers.searchers.BaseSearcher` constructor
    :return: New searcher object
    """
    supported_schedulers = None
    scheduler = kwargs.get("scheduler")
    model = kwargs.get("model", "gp_multitask")
    if not isinstance(searcher_name, str):
        searcher_cls = searcher_name
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.kde import (
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.bore import (
                Bore,
                MultiFidelityBore,
            )
        except ImportError:
        ...
        try:
            from syne_tune_local.optimizer.schedulers.searchers.botorch import (
                BoTorchSearcher,
            )
        except ImportError:
        ...
        try:
            ...
            from syne_tune_local.optimizer.schedulers.searchers.constrained import (
                ConstrainedGPFIFOSearcher,
            )
            from syne_tune_local.optimizer.schedulers.searchers.cost_aware import (
                CostAwareGPFIFOSearcher,
                CostAwareGPMultiFidelitySearcher,
            )
            from syne_tune_local.optimizer.schedulers.searchers.hypertune import (
                HyperTuneSearcher,
            )
            from syne_tune_local.optimizer.schedulers.searchers.dyhpo import (
                DynamicHPOSearcher,
            )
        except ImportError:
