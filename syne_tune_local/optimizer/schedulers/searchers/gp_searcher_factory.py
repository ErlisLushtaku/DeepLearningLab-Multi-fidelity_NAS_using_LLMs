...
from syne_tune_local.optimizer.schedulers.searchers.gp_searcher_utils import (
...
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.kernel_factory import (
    SUPPORTED_BASE_MODELS,
    base_kernel_factory,
    SUPPORTED_RESOURCE_MODELS,
    resource_kernel_factory,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.acqfunc_factory import (
    acquisition_function_factory,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.datatypes.config_ext import (
    ExtendedConfiguration,
)
from syne_tune_local.optimizer.schedulers.searchers.utils import (
    HyperparameterRanges,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.constants import (
    OptimizationConfig,
    DEFAULT_OPTIMIZATION_CONFIG,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.gp_regression import (
    GaussianProcessRegression,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.kernel import (
    KernelFunction,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.warping import (
    kernel_with_warping,
    WarpedKernel,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.mean import (
    MeanFunction,
    ScalarMeanFunction,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.learncurve.freeze_thaw import (
    ExponentialDecayBaseKernelFunction,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.learncurve.model_params import (
    IndependentISSModelParameters,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.learncurve.gpiss_model import (
    GaussianProcessLearningCurveModel,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.independent.gpind_model import (
    IndependentGPPerResourceModel,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.hypertune.gp_model import (
    HyperTuneIndependentGPModel,
    HyperTuneJointGPModel,
    HyperTuneDistributionArguments,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.gpautograd.target_transform import (
    BoxCoxTargetTransform,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.model_skipopt import (
    SkipNoMaxResourcePredicate,
    SkipPeriodicallyPredicate,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.gp_model import (
    GaussProcEmpiricalBayesEstimator,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.gpiss_model import (
    GaussProcAdditiveEstimator,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.cost_fifo_model import (
    CostEstimator,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.meanstd_acqfunc_impl import (
    CEIAcquisitionFunction,
    EIpuAcquisitionFunction,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.tuning_algorithms.defaults import (
...
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.utils.debug_log import (
    DebugLogPrinter,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.subsample_state_multi_fidelity import (
    SubsampleMultiFidelityStateConverter,
    SubsampleMFDenseDataStateConverter,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.subsample_state_single_fidelity import (
    SubsampleSingleFidelityStateConverter,
)
from syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.model_transformer import (
...
from syne_tune_local.optimizer.schedulers.searchers.utils.warmstarting import (
    create_hp_ranges_for_warmstarting,
    create_filter_observed_data_for_warmstarting,
    create_base_gp_kernel_for_warmstarting,
)
from syne_tune_local.optimizer.schedulers.searchers import extract_random_seed
from syne_tune_local.optimizer.schedulers.random_seeds import RANDOM_SEED_UPPER_BOUND
...
def gp_fifo_searcher_factory(**kwargs) -> Dict[str, Any]:
    """
    Returns ``kwargs`` for
    :meth:`~syne_tune_local.optimizer.schedulers.searchers.GPFIFOSearcher._create_internal`,
    based on ``kwargs`` equal to ``search_options`` passed to and extended by
    scheduler (see :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`).
    ...
    * ``max_epochs``: Maximum resource value
    ...
    :return: ``kwargs`` for :meth:`~syne_tune_local.optimizer.schedulers.searchers.GPFIFOSearcher._create_internal`
    """
    assert (
...
def gp_multifidelity_searcher_factory(**kwargs) -> Dict[str, Any]:
    """
    Returns ``kwargs`` for
    :meth:`~syne_tune_local.optimizer.schedulers.searchers.GPMultiFidelitySearcher._create_internal`,
    based on ``kwargs`` equal to ``search_options`` passed to and extended by
    scheduler (see :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`).

    :param kwargs: ``search_options`` coming from scheduler
    :return: ``kwargs`` for :meth:`~syne_tune_local.optimizer.schedulers.searchers.GPMultiFidelitySearcher._create_internal`
    """
    supp_schedulers = {
        "hyperband_stopping",
        "hyperband_promotion",
...
def hypertune_searcher_factory(**kwargs) -> Dict[str, Any]:
    """
    Returns ``kwargs`` for
    :meth:`~syne_tune_local.optimizer.schedulers.searchers.hypertune.HyperTuneSearcher._create_internal`,
    based on ``kwargs`` equal to ``search_options`` passed to and extended by
    scheduler (see :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`).

    :param kwargs: ``search_options`` coming from scheduler
    :return: ``kwargs`` for :meth:`~syne_tune_local.optimizer.schedulers.searchers.hypertune.HyperTuneSearcher._create_internal`
    """
    if kwargs.get("model") is None:
...
def constrained_gp_fifo_searcher_factory(**kwargs) -> Dict[str, Any]:
    """
    Returns ``kwargs`` for
    :meth:`~syne_tune_local.optimizer.schedulers.searchers.constrained.ConstrainedGPFIFOSearcher._create_internal`,
    based on ``kwargs`` equal to ``search_options`` passed to and extended by
    scheduler (see :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`).

    :param kwargs: ``search_options`` coming from scheduler
    :return: ``kwargs`` for :meth:`~syne_tune_local.optimizer.schedulers.searchers.constrained.ConstrainedGPFIFOSearcher._create_internal`
    """
    assert (
        kwargs["scheduler"] == "fifo"
    ), "This factory needs scheduler = 'fifo' (instead of '{}')".format(
...
def cost_aware_coarse_gp_fifo_searcher_factory(**kwargs) -> Dict[str, Any]:
    """
    Returns ``kwargs`` for
    :meth:`~syne_tune_local.optimizer.schedulers.searchers.cost_aware.CostAwareGPFIFOSearcher._create_internal`,
    based on ``kwargs`` equal to ``search_options`` passed to and extended by
    scheduler (see :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`).
    ...
    :param kwargs: ``search_options`` coming from scheduler
    :return: ``kwargs`` for :meth:`~syne_tune_local.optimizer.schedulers.searchers.cost_aware.CostAwareGPFIFOSearcher._create_internal`
    """
    assert (
        kwargs["scheduler"] == "fifo"
    ), "This factory needs scheduler = 'fifo' (instead of '{}')".format(
...
def cost_aware_fine_gp_fifo_searcher_factory(**kwargs) -> Dict[str, Any]:
    ...
    :class:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.cost.cost_model.CostModel`
    surrogate model.
...
def cost_aware_gp_multifidelity_searcher_factory(**kwargs) -> Dict[str, Any]:
    """
    Returns ``kwargs`` for
    :meth:`~syne_tune_local.optimizer.schedulers.searchers.cost_aware.CostAwareGPMultiFidelitySearcher._create_internal`,
    based on ``kwargs`` equal to ``search_options`` passed to and extended by
    scheduler (see :class:`~syne_tune_local.optimizer.schedulers.HyperbandScheduler`).

    :param kwargs: ``search_options`` coming from scheduler
    :return: ``kwargs`` for :meth:`~syne_tune_local.optimizer.schedulers.searchers.cost_aware.CostAwareGPMultiFidelitySearcher._create_internal`
    """
    supp_schedulers = {
        "hyperband_stopping",
        "hyperband_promotion",
...
def gp_fifo_searcher_defaults(kwargs: Dict[str, Any]) -> (Set[str], dict, dict):
    ...
    :func:`~syne_tune_local.optimizer.schedulers.searchers.utils.default_arguments.check_and_merge_defaults`
    to be applied to ``search_options`` for
    :class:`~syne_tune_local.optimizer.schedulers.searchers.GPFIFOSearcher`.
...
def gp_multifidelity_searcher_defaults(
    ...
    :func:`~syne_tune_local.optimizer.schedulers.searchers.utils.default_arguments.check_and_merge_defaults`
    to be applied to ``search_options`` for
    :class:`~syne_tune_local.optimizer.schedulers.searchers.GPMultiFidelitySearcher`.
...
def hypertune_searcher_defaults(kwargs: Dict[str, Any]) -> (Set[str], dict, dict):
    ...
    :func:`~syne_tune_local.optimizer.schedulers.searchers.utils.default_arguments.check_and_merge_defaults`
    to be applied to ``search_options`` for
    :class:`~syne_tune_local.optimizer.schedulers.searchers.hypertune.HyperTuneSearcher`.
...
def constrained_gp_fifo_searcher_defaults(
    ...
    """
    Returns ``mandatory``, ``default_options``, ``config_space`` for
    :func:`~syne_tune_local.optimizer.schedulers.searchers.utils.default_arguments.check_and_merge_defaults` to be applied to ``search_options`` for
    :class:`~syne_tune_local.optimizer.schedulers.searchers.constrained.ConstrainedGPFIFOSearcher`.
...
def cost_aware_gp_fifo_searcher_defaults(
    ...
    :func:`~syne_tune_local.optimizer.schedulers.searchers.utils.default_arguments.check_and_merge_defaults`
    to be applied to ``search_options`` for
    :class:`~syne_tune_local.optimizer.schedulers.searchers.cost_aware.CostAwareGPFIFOSearcher`.
...
def cost_aware_gp_multifidelity_searcher_defaults(
    ...
    :func:`~syne_tune_local.optimizer.schedulers.searchers.utils.default_arguments.check_and_merge_defaults`
    to be applied to ``search_options`` for
    :class:`~syne_tune_local.optimizer.schedulers.searchers.cost_aware.CostAwareGPMultiFidelitySearcher`.
