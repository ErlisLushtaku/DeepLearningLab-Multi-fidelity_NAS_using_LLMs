...
from syne_tune_local.optimizer.schedulers.searchers.model_based_searcher import (
...
class GPFIFOSearcher(BayesianOptimizationSearcher):
    ...
    :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler`. It provides
    ...
    :class:`~syne_tune_local.optimizer.schedulers.FIFOScheduler` objects with
    ``searcher="bayesopt"``, and passing arguments here in ``search_options``.
    This will use the appropriate functions from
    :mod:``syne_tune_local.optimizer.schedulers.searchers.gp_searcher_factory`` to
    create components in a consistent way.

    Most of the implementation is generic in
    :class:`~syne_tune_local.optimizer.schedulers.searchers.model_based_searcher.BayesianOptimizationSearcher`.
    ...
    :mod:``syne_tune_local.optimizer.schedulers.searchers.gp_searcher_factory``. In
    particular, see
    :func:`~syne_tune_local.optimizer.schedulers.searchers.gp_searcher_factory.gp_fifo_searcher_defaults`
    for default values.

    Additional arguments on top of parent class
    :class:`~syne_tune_local.optimizer.schedulers.searchers.StochasticSearcher`:
    ...
    :param num_init_random: Number of initial :meth:`get_config` calls for which
        ...
        :const:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.tuning_algorithms.defaults.DEFAULT_NUM_INITIAL_RANDOM_EVALUATIONS`
    ...
    :param num_init_candidates: Number of initial candidates sampled at
        ...
        Defaults to :const:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.tuning_algorithms.defaults.DEFAULT_NUM_INITIAL_CANDIDATES`
    ...
    :param input_warping: If ``True``, we use a warping transform, so the kernel
        ...
        :class:`~syne_tune_local.optimizer.schedulers.searcher.bayesopt.gpautograd.warping.Warping`.
    ...
    :param gp_base_kernel: Selects the covariance (or kernel) function to be
        ...
        :const:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.kernel_factory.SUPPORTED_BASE_MODELS`.
    ...
    :param acq_function: Selects the acquisition function to be used. Supported
        choices are
        :const:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.acqfunc_factory.SUPPORTED_ACQUISITION_FUNCTIONS`.
    ...
    :param initial_scoring: Scoring function to rank initial candidates
        ...
        * "acq_func": score is the same (EI) acquisition function which is
        ...
        Defaults to
        :const:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.tuning_algorithms.defaults.DEFAULT_INITIAL_SCORING`
    ...
    :param max_size_data_for_model: If this is set, we limit the number of
        ...
        :class:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.models.subsample_state.SubsampleSingleFidelityStateConverter`
        ...
        :const:`~syne_tune_local.optimizer.schedulers.searchers.bayesopt.tuning_algorithms.defaults.DEFAULT_MAX_SIZE_DATA_FOR_MODEL`.
    ...
    :param transfer_learning_task_attr: Used to support transfer HPO, where
        ...
        :class:`~syne_tune_local.optimizer.schedulers.searchers.utils.HyperparameterRanges`.
